  ...
  return (
    ...
    >
      ...
      >
        {!isOwn && (
          <p
            className={`${getUsernameColor(message.user.alias)} mb-1 text-xs font-semibold sm:text-sm`}
          >
            {message.user.alias}
          </p>
        )}
        ...
        >
          ...
          {isOwn && message.status === 'pending' && <BiTimeFive className="ml-1 inline-block" size={15} />}
        </p>
      </div>
    </div>
  );
